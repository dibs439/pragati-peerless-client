...
const HospitalDashboardGraph = ({ propertyId, propertyCode, selectedYear }) => {
  ...
  const getMISData = async () => {
    ...
    //"/",
    ///mis?propertyCode=PHL&year=2024
    let misYear = Number(selectedYear.split("-")[1]);
    const response = await fetch(
      `/mis/new?propertyCode=${propertyCode}&year=${ misYear }`,
      {
    //  const response = await fetch(`/mis?propertyCode=${propertyCode}&year=${ misYear }`, {
    // const response = await fetch(
    //   `/mis?propertyCode=PHH&year=2024`,
    //   {
        method: "GET",
  ...
  const getLastValue = (arr) => (arr.length ? arr[arr.length - 1] : 0);

  const getPercentage = (actual, target) =>
    target ? Math.round((actual / target) * 100) : 0;
  //
          ...
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <Card sx={{ minWidth: 250, height: "100%" }}>
              ...
              <CardContent>
                ...
                <Typography sx={{ fontSize: 16 }} color="text.secondary">
                  ₹{
                    getLastValue(myPageData.thisYearARPOBActualYTD)
                  }
                  {endYear === YTD_year ? "(YTD)" : ""}
                </Typography>
                <Divider></Divider>
                ...
                <Typography sx={{ fontSize: 16 }} color="text.secondary">
                  {
                    getLastValue(myPageData.thisYearALOSActualYTD)
                  }
                  {" days."}
                  {endYear === YTD_year ? "(YTD)" : ""}
                </Typography>
