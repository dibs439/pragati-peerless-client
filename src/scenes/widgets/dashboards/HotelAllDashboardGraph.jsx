...
const HotelAllDashboardGraph = ({ propertyId, selectedYear }) => {
  ...
  const getMISData = async () => {
    ...
    let responseURL ="/mis/cons?propertyCode=PHL&year=2025"
    //responseURL = `/mis/cons?propertyCode=${propertyCode}&year=${misYear}`
    responseURL = `/mis/cons?propertyCode=PHL&year=${misYear}`
    
    const response = await fetch(responseURL,
      {
        //const response = await fetch(`/mis?propertyCode=${propertyCode}&year=${new Date().getFullYear() + 1}`, {
        method: "GET",
